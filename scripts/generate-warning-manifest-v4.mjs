...
function meteoAlarmSystem(countryCode) {
  ...
  return {
    ...
    endpoint: `https://feeds.meteoalarm.org/feeds/meteoalarm-legacy-atom-${slug}`, format: "atom", cadenceMinutes: active ? 30 : 10,
...
noSystems.unshift({
  ...
  format: "json", cadenceMinutes: 30, maxBytes: 2097152, hazards: ["severe-weather", "coastal", "snow-ice", "fire-danger"],
...
gbSystems.push({
  ...
  format: "atom", cadenceMinutes: 30, maxBytes: 2097152, hazards: ["severe-weather", "extreme-heat", "extreme-cold", "snow-ice", "flood", "coastal"],
...
Object.assign(dwd, {
  ...
  runtimeTarget: "meteoalarm-fallback", role: "fallback", status: "active", cadenceMinutes: 30,
