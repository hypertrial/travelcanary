...
import { IngestionStateSchema, downgradeIngestionStateV11, downgradeIngestionStateV12 } from "../src/lib/domain/schemas";
...
const projected = downgradeIngestionStateV11(downgradeIngestionStateV12(state));
const digest = createHash("sha256").update(raw).digest("hex");
const backup = `ingestion-state-v12-before-v10-rollback-${digest}.json`;
...
console.log(JSON.stringify({ applied: process.argv.includes("--apply"), backup, from: 12, to: 10, preservedEvents: projected.events.length, preservedCandidates: projected.candidates.length, bytes: Buffer.byteLength(JSON.stringify(projected)) }));
