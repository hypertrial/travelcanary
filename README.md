...
The hosted adapter uses Vercel Cron Jobs and two Vercel Blob stores. Met Office, NASA FIRMS, reviewed Andorra/Iceland MeteoAlarm EDR recovery, and Natural Resources Wales are optional credentialed transports. Met Office, EDR, and NRW do not add capability credit unless a later reviewed release explicitly promotes them. Public operator guidance deliberately excludes private hosted credential values and project settings.
...
`/api/v1/health` validates the selected snapshot, exact catalog membership, all expected country-condition files, release identity, coverage floors, and viable required transports without calling upstream providers. It returns 503 for an invalid or stale publication or an exhausted required scope; optional credential-gated transports are health-neutral. It exposes only aggregate provider/country/transport identifiers and revalidates its CDN response within 60 seconds.
