...
The hosted adapter uses Vercel Cron Jobs and two Vercel Blob stores. Catalog 3 production requires server-only `MET_OFFICE_API_KEY` and `MET_OFFICE_WARNINGS_FEED_URL` values for complete UK warning coverage. NASA FIRMS, reviewed Andorra/Iceland MeteoAlarm EDR recovery, and Natural Resources Wales remain optional credentialed transports; EDR and NRW do not add capability credit. Public operator guidance deliberately excludes private hosted credential values and project settings.
...
`/api/v1/health` validates the selected snapshot, exact catalog membership, all expected country-condition files, release identity, coverage floors, and viable required transports without calling upstream providers. It returns 503 for an invalid/stale publication, a missing/failed/stale required Met Office transport, or another exhausted required scope; it exposes only aggregate provider/country/transport identifiers and revalidates its CDN response within 60 seconds.
