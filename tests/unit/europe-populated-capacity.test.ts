...
describe("populated expanded conditions capacity", () => {
  it("reproduces full forecasts and retained specialist activity inside unchanged budgets", () => {
    ...
    expect(IngestionStateV16Schema.safeParse(state).success).toBe(true);
    ...
    expect(metrics.publication).toEqual({ conditionsObjects: 45, payloadObjects: 46, immutableObjects: 47, pointerCas: 1 });
  });
});
