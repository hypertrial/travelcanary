...
describe("revision-fenced catalog3 control transition", () => {
  ...
  it.each([-1, 0])("activates without waiting for an existing lease at offset%s", async (offset) => {
    ...
    const store = new MemoryStateStore(state); const write = vi.spyOn(store, "write");
    expect(await activateCatalog3(store, now)).toMatchObject({ status: "activated" });
