...
Liveness is constant and dependency-free. Data health validates the current pointer, manifest, object digests, 679-member snapshot, exactly 45 fresh conditions objects, release identity, and coverage contract. It returns HTTP 200 with a degraded status for reviewed upstream failure while last-good publication remains valid; integrity or freshness failure returns 503. Unavailable 503 responses send `Cache-Control: no-store` and are cached in-process for 5 seconds. `publication.code` is `publication_unreachable` when publication storage fetch rejects, times out, or returns a non-OK non-404 response, and `publication_invalid` for parse, schema, digest, or identity failures. The production verifier treats both codes as release blockers and additionally blocks delayed required life-safety monitoring. Air-quality delays are warnings, and CEMS is context-only: it cannot satisfy monitoring, fail a required transport, or degrade public availability.

Risk and monitoring freshness are independent. Active warning severity determines the risk level. With no active warning, a delayed life-safety check produces `UNKNOWN`; a delayed non-life-safety check leaves the risk `NORMAL` and appears under “Checks delayed.” Global snapshot `dataHealth` and the Live chrome follow blocking-source freshness; destination-scoped delays, including expanded Catalog 3 air-quality holes, do not rewrite that aggregate into the site-wide delayed banner. “Live updates unavailable” is reserved for an invalid or unreachable publication. Permanent capability gaps appear as “Monitoring unavailable,” and failed conditions collection appears as “Local conditions unavailable.”
