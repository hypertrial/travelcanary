...
Core and Catalog 3 country transports share one `deriveTransportState` path. Both callers pass the same input (`system`, `health`, `fallbackStatus`, `effectiveStatus`, `now`). Coverage transports become `delayed` through the same cadence plus `transportIsDelayed` rule. Published transport JSON always includes `lastSuccess` and `nextExpectedUpdate`, using `null` when unknown. A disabled transport uses `system.limitationCode`, or `credential_not_configured` when that value is null. Fallback-role transports stay `ok` while delayed-adjusted `effective.status` is `ok`, so a healthy core fallback does not flip when its own health is `not_monitored` or `failed`.
