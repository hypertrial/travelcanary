...
export function AlertsView({ presentation, filter, onSelect, onShowMap }: {
  ...
  const groups = filter === "unavailable"
    ? [{ key: "unavailable" as const, label: "Checks delayed", level: "UNKNOWN" as const, items: presentation.delayedItems }]
    : presentation.groups.filter((group) => filter === "all" ? group.key !== "unavailable"
      : group.key === "emergency" || group.key === "change-plans");
  const total = groups.reduce((sum, group) => sum + group.items.length, 0);
  const heading = filter === "unavailable" ? "Checks delayed" : filter === "high" ? "High and Severe alerts" : "Current alerts";
  ...
  return <section className={styles.view} aria-labelledby="alerts-heading">
    ...
    {presentation.globalUnavailable ? <div className={styles.empty} data-tone="unavailable"><b>?</b><h3>Live updates unavailable</h3><p>Current alerts could not be confirmed. Search a destination or check official local sources.</p></div>
      : total === 0 ? <div className={styles.empty}><b><UiIcon name="search" /></b><h3>{filter === "high" ? "No High or Severe alerts found" : filter === "unavailable" ? "No delayed checks" : "No destinations are flagged"}</h3><p>This is limited to checked sources and is not an all-clear.</p><button type="button" onClick={onShowMap}>Return to map</button></div>
